...
namespace MemoryCommando::Memory::Internal {
    ...
    public:
        ...
        uintptr_t AllocateVirtualMemory(uintptr_t baseAddress, size_t allocationSize, DWORD protectionType = PAGE_EXECUTE_READWRITE, DWORD allocationType = MEM_RESERVE | MEM_COMMIT) const override;
        void FreeVirtualMemory(uintptr_t address, DWORD freeType = MEM_RELEASE, size_t size = 0) const override;
        void ProtectVirtualMemory(uintptr_t address, size_t protectionSize, DWORD protectionType = PAGE_EXECUTE_READWRITE) const override;
        MEMORY_BASIC_INFORMATION QueryVirtualMemory(uintptr_t address) const override;
